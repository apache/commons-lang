...
Building requires a Java JDK. Use the included Maven Wrapper to download and run [Apache Maven](https://maven.apache.org/).
The required Java version is found in the `pom.xml` as the `maven.compiler.source` property.

From a command shell, run `./mvnw` without arguments to invoke the default Maven goal to run all tests and checks.
...
+ Provide JUnit tests for your changes and make sure your changes don't break any existing tests by running `./mvnw`.
+ Before you push a PR, run `./mvnw` (without arguments). This runs the default goal which contains all build checks.
+ To see the code coverage report, regardless of coverage failures, run `./mvnw clean site -Dcommons.jacoco.haltOnFailure=false -Pjacoco`
