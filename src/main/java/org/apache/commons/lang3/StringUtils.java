...
public class StringUtils {
    ...
    /**
     * Refactoring note:
     * The implementation is behavior-identical to the previous version, but split into small helper
     * methods to reduce cyclomatic complexity and make the main method a high-level orchestrator.
     */
    @Deprecated
    public static int getLevenshteinDistance(CharSequence s, CharSequence t, final int threshold) {
        validateLevenshteinArgs(s, t, threshold);

        int n = s.length(); // length of s
        int m = t.length(); // length of t

        // Fast-path exits (empty strings / impossible due to length difference).
        final int early = levenshteinEarlyReturn(n, m, threshold);
        if (early != Integer.MIN_VALUE) {
            return early;
        }

        // Ensure s is the shorter sequence to reduce memory usage (as in original implementation).
        final LevenshteinInput in = ensureShorterInput(s, t, n, m);
        s = in.s;
        t = in.t;
        n = in.n;
        m = in.m;

        final int[] p = initPreviousRow(n, threshold);
        final int[] d = initCurrentRow(n);

        final int distance = computeLevenshteinDistanceWithinThreshold(s, t, n, m, threshold, p, d);
        return distance != -1 && distance <= threshold ? distance : -1;
    }
    ...
    /**
     * Handles cheap early-return cases.
     * Returns Integer.MIN_VALUE if no early return applies (caller should continue).
     */
    ...
    /**
     * Ensures the first sequence is the shorter one (same optimization as original code).
     * IMPORTANT: accepts the already-computed lengths to avoid extra length() calls.
     */
    private static LevenshteinInput ensureShorterInput(CharSequence s, CharSequence t, int n, int m) {
        if (n <= m) {
            return new LevenshteinInput(s, t, n, m);
        }
        ...
        // after swap: n becomes old m; m becomes new t length (old s length)
        final int newN = m;
        final int newM = t.length();
        return new LevenshteinInput(s, t, newN, newM);
    }

    /** Initializes the "previous row" cost array (p) with the diagonal stripe boundary rule. */
    private static int[] initPreviousRow(final int n, final int threshold) {
        final int[] p = new int[n + 1];
        ...
        // values above the rightmost stripe entry must be ignored
        Arrays.fill(p, boundary, p.length, Integer.MAX_VALUE);
        return p;
    }

    /** Initializes the "current row" cost array (d) to MAX so non-computed entries are ignored. */
    private static int[] initCurrentRow(final int n) {
    ...
    /**
     * Core DP loop restricted to a diagonal stripe of width (2*threshold + 1).
     * Returns -1 if the stripe runs off the matrix (no possible solution within threshold),
     * otherwise returns the computed distance (caller applies final threshold check).
     */
    private static int computeLevenshteinDistanceWithinThreshold(
        ...
        for (int j = 1; j <= m; j++) {
            ...
            final int min = stripeMinIndex(j, threshold);
            final int max = stripeMaxIndex(n, j, threshold);
            ...
            for (int i = min; i <= max; i++) {
                d[i] = (s.charAt(i - 1) == jOfT)
                        ...
                        : 1 + Math.min(Math.min(d[i - 1], p[i]), p[i - 1]); // left, top, diag
            }
    ...
    private static int stripeMinIndex(final int j, final int threshold) {
        return Math.max(1, j - threshold);
    }

    private static int stripeMaxIndex(final int n, final int j, final int threshold) {
        // preserve the original overflow guard behavior
        return j > Integer.MAX_VALUE - threshold ? n : Math.min(n, j + threshold);
    }

    private static final class LevenshteinInput {
        final CharSequence s;
        final CharSequence t;
        final int n;
        final int m;

        LevenshteinInput(final CharSequence s, final CharSequence t, final int n, final int m) {
            this.s = s;
            this.t = t;
            this.n = n;
            this.m = m;
