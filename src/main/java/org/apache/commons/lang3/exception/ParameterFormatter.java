...
/**
 * Supports parameter formatting as used in {@link ParameterizedException}.
 */
final class ParameterFormatter {
  ...
  static final FastDateFormat FORMAT =
   ...
   * @param arguments the arguments to be used to replace placeholders.
   */
  static void formatMessage(final StringBuilder buffer,
  ...
  private static boolean isDelimPair(final char curChar,
      final String messagePattern, final int curCharIndex) {
    return curChar == DELIM_START
        && messagePattern.charAt(curCharIndex + 1) == DELIM_STOP;
  }

  /**
   * Detects whether the message pattern has been fully processed or if an
   * unprocessed character remains and processes it if necessary, returning the
   * resulting position in the result char array.
   */
  // Profiling showed this method is important to log4j performance. Modify with
  // care!
  ...
  private static void handleRemainingCharIfAny(final String messagePattern,
      final int len, final StringBuilder buffer, final int escapeCounter,
      final int i) {
    if (i == len - 1) {
      final char curChar = messagePattern.charAt(i);
  ...
  /**
   * Processes the last unprocessed character and returns the resulting position
   * in the result char array.
   */
  // Profiling showed this method is important to log4j performance. Modify with
  // care!
  // 28 bytes (allows immediate JVM inlining: < 35 bytes) LOG4J2-1096
  private static void handleLastChar(final StringBuilder buffer,
  ...
  private static void handleLiteralChar(final StringBuilder buffer,
      final int escapeCounter, final char curChar) {
  ...
  /**
   * Writes a '\' char to the specified result array (starting at the specified
   * position) for each <em>pair</em> of '\' escape chars encountered in the
   * message format and returns the resulting position.
   */
  // Profiling showed this method is important to log4j performance. Modify with
  // care!
  // 11 bytes (allows immediate JVM inlining: < 35 bytes) LOG4J2-1096
  private static void writeEscapedEscapeChars(final int escapeCounter,
  ...
  private static void writeArgOrDelimPair(final Object[] arguments,
      final int argCount, final int currentArgument,
      final StringBuilder buffer) {
  ...
  /**
   * This method performs a deep toString of the given Object. Primitive arrays
   * are converted using their respective Arrays.toString methods while special
   * handling is implemented for "container types", i.e. Object[], Map and
   * Collection because those could contain themselves.
   * <p>
   * It should be noted that neither AbstractMap.toString() nor
   * AbstractCollection.toString() implement such a behavior. They only check if
   * the container is directly contained in itself, but not if a contained
   * container contains the original one. Because of that,
   * Arrays.toString(Object[]) isn't safe either. Confusing? Just read the last
   * paragraph again and check the respective toString() implementation.
   * </p>
   * <p>
   * This means, in effect, that logging would produce a usable output even if
   * an ordinary System.out.println(o) would produce a relatively hard-to-debug
   * StackOverflowError.
   * </p>
   * 
   * @param o The object.
   ...
   */
  private static void recursiveDeepToString(final Object o,
      final StringBuilder str, final Set<String> dejaVu) {
  ...
  private static boolean appendSpecialTypes(final Object o,
  ...
  private static boolean isMaybeRecursive(final Object o) {
    return o.getClass().isArray() || o instanceof Map
  ...
  private static void appendArray(final Object o, final StringBuilder str,
      Set<String> dejaVu, final Class<?> oClass) {
  ...
  private static void tryObjectToString(final Object o,
      final StringBuilder str) {
  ...
  private static void handleErrorInObjectToString(final Object o,
      final StringBuilder str, final Throwable t) {
   ...
   * overridden in obj.
   * <p>
   * Note that this isn't 100% secure as collisions can always happen with hash
   * codes.
   * </p>
   * <p>
   ...
   * defined by class {@code Object} does return distinct integers for distinct
   ...
   * required by the Java&#8482; programming language.) </blockquote>
   *
